...
std::shared_ptr<T> make_input_stream_with_sample_filter_(
    ...
    if( ! sample_indices.empty() ) {
        ...
        input = std::make_shared<T>( genesis::util::io::from_file( filename ), reader );
        ...
        auto smp_flt = genesis::util::bit::make_bool_vector_from_indices(
        ...
        input = std::make_shared<T>(
            genesis::util::io::from_file( filename ),
            smp_flt,
            reader
        );
...
VariantInputStream make_variant_input_stream_from_sam_file(
    ...
    data.source_name = genesis::util::core::file_basename( filename, { ".sam", ".sam.gz", ".bam", ".cram" });
...
VariantInputStream make_variant_input_stream_from_pileup_file_(
    ...
    VariantInputStreamData data;
    data.file_path = filename;
    data.source_name = genesis::util::core::file_basename(
...
VariantInputStream make_variant_input_stream_from_sync_file_(
    ...
    // auto input = std::make_shared<SyncInputStream>( genesis::util::io::from_file( filename ));

    // Get the data, using the file base name without path and potential extensions as source.
    VariantInputStreamData data;
    data.file_path = filename;
    data.source_name = genesis::util::core::file_basename( filename, { ".gz", ".sync" });
...
VariantInputStream make_variant_input_stream_from_frequency_table_file(
    ...
    input->input_source( genesis::util::io::from_file( filename ));
...
VariantInputStream make_variant_input_stream_from_vcf_file_(
    ...
    VariantInputStreamData data;
    data.file_path = filename;
    data.source_name = genesis::util::core::file_basename( filename, { ".gz", ".vcf", ".bcf" });
