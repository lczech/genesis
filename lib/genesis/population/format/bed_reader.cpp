...
size_t BedReader::parse_line_(
    ...
    while( is_comment ) {
        ...
        if(
            first_word == "browser" ||
            first_word == "track" ||
            genesis::util::text::starts_with( first_word, "#" )
        ) {
    ...
    feature.chrom_start = genesis::util::io::parse_unsigned_integer<size_t>( it ) + 1;
    if( ! next_field_( it, found_columns )) {
        throw std::runtime_error(
            "BED input expected to have three mandatory columns chrom,start,end in the beginning "
    ...
    feature.chrom_end = genesis::util::io::parse_unsigned_integer<size_t>( it );
    ...
    feature.score = genesis::util::io::parse_unsigned_integer<size_t>( it );
    ...
    feature.strand = genesis::util::io::read_char_or_throw( input_stream, []( char c ){
        return c == '+' || c == '-' || c == '.';
    });
    ...
    feature.thick_start = genesis::util::io::parse_unsigned_integer<size_t>( it ) + 1;

    // thick_end
    if( ! next_field_( it, found_columns )) {
        return found_columns;
    }
    feature.thick_end = genesis::util::io::parse_unsigned_integer<size_t>( it );
    ...
    feature.block_count = genesis::util::io::parse_unsigned_integer<size_t>( it );

    // block_sizes
    if( ! next_field_( it, found_columns )) {
        return found_columns;
    }
    auto block_sizes = genesis::util::text::split( parse_string_( it ), "," );
    ...
    auto block_starts = genesis::util::text::split( parse_string_( it ), "," );
    for( auto const& bs : block_sizes ) {
        if( ! bs.empty() ) {
...
bool BedReader::next_field_( genesis::util::io::InputStream& input_stream, size_t& found_columns ) const
{
    ...
    assert( input_stream && *input_stream != '\n' );
    genesis::util::io::read_char_or_throw( input_stream, []( char c ){
        return c == '\t' || c == ' ';
    });
    genesis::util::io::skip_while( input_stream, []( char c ){
        return c == '\t' || c == ' ';
    });
    if( ! input_stream || *input_stream == '\n' ) {
...
std::string BedReader::parse_string_( genesis::util::io::InputStream& input_stream ) const
{
    // We use \n as stopping criterion here as well, so that in case of an error,
    // we at least report the error in the correct line.
    return genesis::util::io::read_while( input_stream, []( char c ){
        return c != '\t' && c != ' ' && c != '\n';
    });
}
