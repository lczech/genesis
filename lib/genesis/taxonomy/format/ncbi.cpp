...
NcbiNodeLookup read_ncbi_node_table(
    ...
    auto get_field = [](
        genesis::util::format::CsvReader::Line& line, size_t pos, std::string const& field_name
    ) -> std::string& {
        if( pos >= line.size() ) {
    ...
    auto csv_iterator = genesis::util::format::CsvInputIterator( source, get_ncbi_file_reader_() );
    ...
    while( csv_iterator ) {
        ...
        if( params.validate_name_characters ) {
            auto const all_print = std::all_of(
                name_entry.name.begin(), name_entry.name.end(), genesis::util::text::is_print
            );
            if( !all_print ) {
                throw std::runtime_error(
...
Taxonomy read_ncbi_taxonomy(
    ...
    return read_ncbi_taxonomy(
        genesis::util::io::from_file( node_file ), genesis::util::io::from_file( name_file ), params
    );
}
