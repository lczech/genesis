#ifndef GENESIS_UTIL_IO_OUTPUT_TARGET_H_
#define GENESIS_UTIL_IO_OUTPUT_TARGET_H_
 ...
 * @link genesis::util::core::Options::allow_file_overwriting( bool ) Options::allow_file_overwriting()@endlink
...
inline std::shared_ptr<BaseOutputTarget> to_file(
    ...
    auto fn = file_name;
    auto const ext = genesis::util::core::file_extension( fn );
...
inline std::shared_ptr<BaseOutputTarget> to_gzip_block_file(
    std::string const& file_name,
    std::shared_ptr<genesis::util::threading::ThreadPool> thread_pool,
