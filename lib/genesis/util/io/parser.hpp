#ifndef GENESIS_UTIL_IO_PARSER_H_
#define GENESIS_UTIL_IO_PARSER_H_
 ...
 */
template<class T>
T parse_unsigned_integer( genesis::util::io::InputStream& source )
{
 ...
 */
template<class T>
T parse_signed_integer( genesis::util::io::InputStream& source )
{
    static_assert(
 ...
 */
template<class T>
T parse_integer( genesis::util::io::InputStream& source )
{
 ...
 */
template<class T>
T parse_float( genesis::util::io::InputStream& source )
{
    T x = 0.0;
    ...
    while( source && genesis::util::text::is_digit( *source )) {
        int y = *source - '0';
        x *= 10;
        x += y;
        ++source;
    ...
    if( source && *source == '.' ) {
        ...
        if( ! source || ! genesis::util::text::is_digit( *source ) ) {
            throw std::runtime_error(
    ...
    if( ! found_mantisse && source ) {
        ...
        if( value != 0.0 && ( !source || ! genesis::util::text::is_alnum( *source ))) {
            return value;
        }
    }
    ...
    // Exponential part
    if( source && genesis::util::text::to_lower(*source) == 'e' ) {
        ++source;
