...
// Forward declaration
size_t parse_unsigned_integer_naive_( genesis::util::io::InputStream& source );

/**
 * @brief Super fast loop-less parsing of unsigned ints from < 8 chars,
 * using GCC/Clang compiler intrinsic builtins.
 */
size_t parse_unsigned_integer_gcc_intrinsic_( genesis::util::io::InputStream& source )
{
...
// # because we are using `__builtin_mul_overflow` in genesis/util/io/input_stream.cpp
 ...
 */
size_t parse_unsigned_integer_from_chars_( genesis::util::io::InputStream& source )
{
    ...
    // using __has_builtin first. See genesis/util/math/bit.hpp for examples.
 ...
 */
size_t parse_unsigned_integer_std_from_chars_( genesis::util::io::InputStream& source )
{
...
size_t parse_unsigned_integer_naive_( genesis::util::io::InputStream& source )
{
    ...
    if( data_pos >= data_end || ! genesis::util::text::is_digit( buffer[ data_pos ] ) ) {
        throw std::runtime_error(
            "Expecting digit in " + source.source_name() + " at " + source.at() + "."
        );
    }

    while(( data_pos < data_end ) && genesis::util::text::is_digit( buffer[ data_pos ] )) {
...
size_t parse_unsigned_integer_size_t( genesis::util::io::InputStream& source )
{
...
std::string parse_number_string(
    genesis::util::io::InputStream& source
) {
    using namespace genesis::util::text;
