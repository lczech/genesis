#ifndef GENESIS_UTIL_IO_INPUT_READER_H_
#define GENESIS_UTIL_IO_INPUT_READER_H_
        ...
        switch( Options::get().input_reading_thread_policy() ) {
            ...
            case Options::InputReadingThreadPolicy::kTrivialAsync: {
                if( input_source->is_trivial() ) {
                    thread_pool_ = std::make_shared<genesis::util::threading::ThreadPool>( 1 );
                } else {
                    thread_pool_ = get_thread_pool_();
                }
                break;
...
private:
    ...
    genesis::util::threading::ProactiveFuture<size_t> future_;
