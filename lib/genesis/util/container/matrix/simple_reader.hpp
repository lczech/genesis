#ifndef GENESIS_UTIL_CONTAINER_MATRIX_SIMPLE_READER_H_
#define GENESIS_UTIL_CONTAINER_MATRIX_SIMPLE_READER_H_
...
public:
    ...
    Matrix<T> read( std::shared_ptr<genesis::util::io::BaseInputSource> source ) const
    {
     ...
     */
    MatrixSimpleReader& parse_value_functor( std::function<T( genesis::util::io::InputStream& )> functor )
    {
...
private:
    ...
    Matrix<T> parse_( genesis::util::io::InputStream& input_stream ) const
    {
    ...
    size_t parse_line_( genesis::util::io::InputStream& input_stream, std::vector<T>& table ) const
    {
        auto& it = input_stream;
        size_t cnt = 0;
        while( it && *it != '\n' ) {
            ...
            if( it && *it != separator_char_ && *it != '\n' ) {
                throw std::runtime_error(
                    "In " + it.source_name() + " at " + it.at() + ": " +
                    "Unexpected char " + genesis::util::text::char_to_hex( *it )
                );
            }
            assert( !it || *it == separator_char_ || *it == '\n' );
    ...
    T parse_field_( genesis::util::io::InputStream& input_stream ) const
    {
        T result;
        auto& it = input_stream;
        if( parse_value_ ) {
            ...
            if( convert_value_ ) {
                ...
                // Expensive, but works.
                try {
                    result = genesis::util::text::convert_from_string<T>( value );
                } catch(...) {
                    throw std::runtime_error(
                        "In " + it.source_name() + " at " + it.at() + ": " +
                        "Invalid value \"" + value + "\""
...
private:
    ...
    std::function<T( genesis::util::io::InputStream& )> parse_value_;
