#ifndef GENESIS_UTIL_FORMAT_NEXUS_TREES_H_
#define GENESIS_UTIL_FORMAT_NEXUS_TREES_H_
...
// class NexusTrees;
// bool operator == ( NexusTrees const& lhs, NexusTrees const& rhs );
// bool operator != ( NexusTrees const& lhs, NexusTrees const& rhs );
...
public:
    ...
    void add_tree( std::string const& name, std::string const& tree )
    {
        ...
        entries_.push_back({ name, std::vector<std::string>{}, tree });
        // entries_.emplace_back(name, {}, tree);
    }

    void erase_tree( std::string const& name )
    {
        genesis::util::core::erase_if(
            entries_,
            [&name] (Entry const& entry) {
                return entry.name == name;
            }
