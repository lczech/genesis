...
JsonDocument JsonReader::read( std::shared_ptr<genesis::util::io::BaseInputSource> source ) const
{
...
JsonDocument JsonReader::parse_value( InputStream& input_stream ) const
{
    ...
    // Parse a number.
    } else if( is_digit( *it ) or ( *it == '+' ) or ( *it == '-' ) or ( *it == '.' )) {
...
JsonDocument JsonReader::parse_number( InputStream& input_stream ) const
{
    ...
    // If not float
    if( found_mantisse && !( *it == '.' || genesis::util::text::to_lower(*it) == 'e' ) ) {
        if( is_neg ) {
    ...
    // Exponential part
    if( it && genesis::util::text::to_lower(*it) == 'e' ) {
        ++it;
