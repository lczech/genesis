...
std::vector<double> Tickmarks::linear_ticks( double min, double max, size_t target_steps )
{
    ...
    if( target_steps > 0 ) {
        ...
        while( tick <= max ) {
            res.push_back( genesis::util::math::round_to( tick, rounding_accuracy ));
            tick += step_sz;
        }
    ...
    if( include_min ) {
        res.push_back( genesis::util::math::round_to( min, rounding_accuracy ));
    }
    if( include_max ) {
        res.push_back( genesis::util::math::round_to( max, rounding_accuracy ));
    }
    ...
    auto uniq_end = std::unique( res.begin(), res.end(), [&]( double lhs, double rhs ){
        return genesis::util::math::almost_equal_relative( lhs, rhs, relative_epsilon );
    });
...
std::vector<Tickmarks::LabeledTick> Tickmarks::logarithmic_labels( double min, double max, double base )
{
    ...
    auto uniq_end = std::unique( res.begin(), res.end(),
        [&]( LabeledTick const& lhs, LabeledTick const& rhs ){
            return genesis::util::math::almost_equal_relative(
