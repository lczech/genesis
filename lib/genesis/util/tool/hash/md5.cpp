...
MD5::DigestType MD5::hex_to_digest( std::string const& hex )
{
    ...
    for (size_t i = 0; i < result.size(); ++i) {
        ...
        std::string const sub = hex.substr( 2*i, 2 );
        char* endptr;
        auto const  res = strtoul( sub.c_str(), &endptr, 16 );
        if( *endptr != 0 ) {
            throw std::runtime_error( "Invalid MD5 hex string: \"" + hex + "\"" );
        }
...
void MD5::update( std::shared_ptr<genesis::util::io::BaseInputSource> source )
{
    auto ib = genesis::util::io::InputBuffer( source );
