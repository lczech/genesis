#ifndef GENESIS_UTIL_TEXT_CONVERT_H_
#define GENESIS_UTIL_TEXT_CONVERT_H_
...
T convert_from_string( std::string const& str, bool trim = false )
{
    ...
    bool good = true;
    try {
        std::stringstream ss( trim ? genesis::util::text::trim(str) : str );
...
inline std::string convert_from_string<std::string>( std::string const& str, bool trim )
{
    ...
    return (trim ? genesis::util::text::trim(str) : str);
}
...
inline double convert_from_string<double>( std::string const& str, bool trim )
{
    ...
    auto const input = (trim ? genesis::util::text::trim(str) : str);
    char* end = nullptr;
