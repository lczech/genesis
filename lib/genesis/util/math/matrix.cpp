...
std::vector<MinMaxPair<double>> normalize_cols(
    ...
    // Iterate the matrix.
    genesis::util::threading::parallel_for( 0, data.cols(), [&]( size_t c )
    {
...
std::vector<MinMaxPair<double>> normalize_rows(
    ...
    // Iterate the matrix.
    genesis::util::threading::parallel_for( 0, data.rows(), [&]( size_t r )
    {
...
std::vector<size_t> filter_constant_columns(
    ...
    auto new_mat = genesis::util::container::Matrix<double>(
...
genesis::util::container::Matrix<double> sums_of_squares_and_cross_products_matrix(
    ...
    auto mat = genesis::util::container::Matrix<double>( data.cols(), data.cols() );
