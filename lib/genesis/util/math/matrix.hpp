#ifndef GENESIS_UTIL_MATH_MATRIX_H_
#define GENESIS_UTIL_MATH_MATRIX_H_
...
MinMaxPair<T> matrix_minmax(
    genesis::util::container::Matrix<T> const& data,
...
std::vector<MinMaxPair<double>> normalize_cols(
    genesis::util::container::Matrix<double>& data
);
 ...
 *
 * The @link genesis::util::container::Matrix Matrix@endlink is modified so that all columns `c` with
 ...
 */
genesis::util::container::Matrix<double> correlation_matrix(
    genesis::util::container::Matrix<double> const& data
);
 ...
 * sums_of_squares_and_cross_products_matrix().
 */
genesis::util::container::Matrix<double> covariance_matrix(
    genesis::util::container::Matrix<double> const& data
);
 ...
 */
genesis::util::container::Matrix<double> sums_of_squares_and_cross_products_matrix(
    genesis::util::container::Matrix<double> const& data
);
 ...
 */
template< typename T >
genesis::util::container::Matrix<T> matrix_sort_by_row_sum_symmetric(
    genesis::util::container::Matrix<T> const& data
) {
    ...
    auto result = genesis::util::container::Matrix<T>( data.rows(), data.cols() );
 ...
 */
template< typename T >
genesis::util::container::Matrix<T> matrix_sort_by_col_sum_symmetric(
    genesis::util::container::Matrix<T> const& data
) {
    if( data.rows() != data.cols() ) {
        throw std::runtime_error( "Symmetric sort only works on square matrices." );
 ...
 */
template< typename T >
genesis::util::container::Matrix<T> matrix_sort_diagonal_symmetric(
    genesis::util::container::Matrix<T> const& data
) {
    if( data.rows() != data.cols() ) {
        throw std::runtime_error( "Symmetric sort only works on square matrices." );
    ...
    auto find_max = []( genesis::util::container::Matrix<T> const& mat, size_t start ){
        size_t max_r = start;
        size_t max_c = start;
...
genesis::util::container::Matrix<T> matrix_addition(
    genesis::util::container::Matrix<A> const& a,
    genesis::util::container::Matrix<B> const& b
) {
    if( a.rows() != b.rows() || a.cols() != b.cols() ) {
        throw std::runtime_error( "Cannot add matrices with different dimensions." );
    }

    auto result = genesis::util::container::Matrix<T>( a.rows(), a.cols() );
...
template< typename T = double, typename A = double, typename B = double >
genesis::util::container::Matrix<T> matrix_addition(
    genesis::util::container::Matrix<A> const& matrix,
    B const& scalar
) {
    auto result = genesis::util::container::Matrix<T>( matrix.rows(), matrix.cols() );
 ...
 */
template< typename T = double, typename A = double, typename B = double >
genesis::util::container::Matrix<T> matrix_subtraction(
    genesis::util::container::Matrix<A> const& a,
    genesis::util::container::Matrix<B> const& b
) {
    if( a.rows() != b.rows() || a.cols() != b.cols() ) {
 ...
 */
template< typename T = double, typename A = double, typename B = double >
genesis::util::container::Matrix<T> matrix_multiplication(
    genesis::util::container::Matrix<A> const& a,
    genesis::util::container::Matrix<B> const& b
) {
    if( a.cols() != b.rows() ) {
    ...
    auto result = genesis::util::container::Matrix<T>( a.rows(), b.cols() );
    for( size_t r = 0; r < a.rows(); ++r ) {
