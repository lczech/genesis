...
genesis::util::container::Matrix<double> p_norm_distance_matrix(
    genesis::util::container::Matrix<double> const& data,
    double p
) {
    // Init result matrix.
    auto result = genesis::util::container::Matrix<double>( data.rows(), data.rows(), 0.0 );

    // We only need to calculate the upper triangle. Get the number of indices needed
    // to describe this triangle.
    size_t const max_k = genesis::util::container::triangular_size( data.rows() );

    // Calculate.
    genesis::util::threading::parallel_for( 0, max_k, [&]( size_t k )
    {
        // For the given linear index, get the actual position in the Matrix.
        auto const ij = genesis::util::container::triangular_indices( k, data.rows() );
...
genesis::util::container::Matrix<double> manhattan_distance_matrix(
    genesis::util::container::Matrix<double> const& data
) {
    return p_norm_distance_matrix( data, 1.0 );
}

genesis::util::container::Matrix<double> euclidean_distance_matrix(
    genesis::util::container::Matrix<double> const& data
) {
    return p_norm_distance_matrix( data, 2.0 );
}

genesis::util::container::Matrix<double> maximum_distance_matrix(
    genesis::util::container::Matrix<double> const& data
) {
