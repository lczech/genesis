#ifndef GENESIS_UTIL_MATH_RANGE_MINIMUM_QUERY_H_
#define GENESIS_UTIL_MATH_RANGE_MINIMUM_QUERY_H_
 ...
 * `std::vector` and genesis::util::container::Matrix, and by replacing C-style casts with proper C++-casts.
...
public:
    ...
    template<class T>
    using Matrix = genesis::util::container::Matrix<T>;
    ...
    explicit RangeMinimumQuery( std::vector<IntType> const& array );
    explicit RangeMinimumQuery( std::vector<IntType>&&      array );
