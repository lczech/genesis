#ifndef GENESIS_UTIL_MATH_HAC_FUNCTION_H_
#define GENESIS_UTIL_MATH_HAC_FUNCTION_H_
 ...
 */
template<typename T>
genesis::util::container::Matrix<double> hac_distance_matrix(
    ...
    auto result = genesis::util::container::Matrix<double>( num_elem, num_elem, 0.0 );
...
std::string hac_dendrogram(
    ...
    for( size_t i = 0; i < clustering.mergers().size(); ++i ) {
        ...
        if( with_branch_lengths ) {
            node_a += ":" + genesis::util::text::to_string_nice( cm.branch_length_a );
            node_b += ":" + genesis::util::text::to_string_nice( cm.branch_length_b );
        }
        list.push_back(
