...
genesis::util::container::Matrix<double> glm_convert_dataframe(
    ...
    auto result = genesis::util::container::Matrix<double>( df.rows(), df.cols() );
    ...
    for( size_t c = 0; c < df.cols(); ++c ) {
        if( ! df[c].genesis::util::container::Dataframe::ColumnBase::is<double>() ) {
            throw std::runtime_error( "GLM Dataframe conversion expects Columns of type double." );
        }
        auto const& col = df[c].genesis::util::container::Dataframe::ColumnBase::as<double>();
