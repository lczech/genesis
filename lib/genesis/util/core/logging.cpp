...
void Logging::log_to_file( std::string const& filename )
{
    ...
    // genesis::util::io::file_output_stream( filename, *file_stream );
    ...
    fstreams_.push_back( genesis::util::core::make_unique<std::ofstream>());
    genesis::util::io::file_output_stream( filename, *fstreams_.back() );
}
...
Logging::~Logging()
{
    ...
    if (msg.length() > 0) {
        msg += genesis::util::text::replace_all(
            buff_.str(), "\n", "\n" + std::string(msg.length(), ' ')
        );
    } else {
        msg += buff_.str();
    }
    msg = genesis::util::text::trim_right(msg);
