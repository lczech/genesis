#ifndef GENESIS_UTIL_CORE_OPTIONS_H_
#define GENESIS_UTIL_CORE_OPTIONS_H_
...
public:
    ...
    void init_global_thread_pool( size_t num_threads, size_t max_queue_size = 0 )
    {
        ...
        assert( num_threads > 0 );
        thread_pool_ = std::make_shared<genesis::util::threading::ThreadPool>( num_threads - 1, max_queue_size );
    }
     ...
     */
    std::shared_ptr<genesis::util::threading::ThreadPool> global_thread_pool() const
    {
        if( ! thread_pool_ ) {
            throw std::runtime_error(
     ...
     *
     * @see @link genesis::util::io::file_output_stream() file_output_stream()@endlink
     */
