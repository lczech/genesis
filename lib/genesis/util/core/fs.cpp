...
{
    std::ofstream ofs;
    genesis::util::io::file_output_stream( filename, ofs, std::ios_base::out, create_dirs );
    ofs << content;
}
...
void dir_create( std::string const& path, bool with_parents )
{
    ...
    auto const path_no_bs = genesis::util::text::trim_right( path, "/\\");
    if( path_no_bs.empty() ) {
        return;
    }
...
{
    return genesis::util::text::trim_right( path, "/") + "/";
}
...
std::string file_basename(
    ...
    for( auto const& ext : remove_extensions ) {
        if( genesis::util::text::ends_with( bn, ext ) ) {
...
bool is_valid_filename( std::string const& filename )
{
    ...
    if( starts_with( filename, " " ) || genesis::util::text::ends_with( filename, " " )) {
        return false;
    }
