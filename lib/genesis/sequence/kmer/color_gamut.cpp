...
size_t KmerColorGamut::add_color( genesis::utils::bit::Bitvector&& elements, bool ignore_duplicates )
{
    ...
    if( (!ignore_duplicates) && genesis::utils::bit::pop_count( elements ) < 2 ) {
        throw std::invalid_argument(
    ...
    auto const hash = genesis::utils::bit::bitvector_hash( elements );
...
size_t KmerColorGamut::find_existing_color( genesis::utils::bit::Bitvector const& target_elements ) const
{
    ...
    auto const target_hash = genesis::utils::bit::bitvector_hash( target_elements );
...
size_t KmerColorGamut::find_minimal_superset( genesis::utils::bit::Bitvector const& target_elements ) const
{
    std::shared_lock read_lock( color_mutex_, std::defer_lock );
    if( ! gamut_started_.load( std::memory_order_acquire )) {
        read_lock.lock();
...
void KmerColorGamut::precompute_gamut( std::shared_ptr<genesis::utils::threading::ThreadPool> thread_pool )
{
...
void KmerColorGamut::init_primary_colors_()
{
    ...
    auto const empty_hash = genesis::utils::bit::bitvector_hash( empty );
...
void KmerColorGamut::populate_target_color_(
    ...
    if( target_elements.empty() ) {
        ...
        target_hash = genesis::utils::bit::bitvector_hash( target_elements );
    } else {
        ...
        assert( target_hash == genesis::utils::bit::bitvector_hash( target_elements ));
    }
}
...
size_t KmerColorGamut::find_existing_color_( genesis::utils::bit::Bitvector const& target, size_t hash ) const
{
    // Sanity checks.
    assert( target.size() == element_count_ );
    assert( hash == genesis::utils::bit::bitvector_hash( target ));
...
size_t KmerColorGamut::add_color_( genesis::utils::bit::Bitvector&& elements, size_t hash )
{
    // Sanity checks.
    assert( colors_.size() == lookup_.size() );
    if( max_color_count_ > 0 && colors_.size() >= max_color_count_ ) {
    ...
    assert( hash == genesis::utils::bit::bitvector_hash( elements ));
...
void KmerColorGamut::init_gamut_()
{
    ...
    gamut_guard_ = genesis::utils::threading::ConcurrentVectorGuard( std::sqrt( colors_.size() * element_count_ ));
    ...
    gamut_ = genesis::utils::containers::Matrix<size_t>( colors_.size(), element_count_, 0 );
...
size_t KmerColorGamut::get_gamut_entry_(
    ...
    {
        genesis::utils::threading::ConcurrentVectorGuard::LockGuard lock;
        if( needs_locking ) {
...
size_t KmerColorGamut::find_minimal_superset_( genesis::utils::bit::Bitvector const& target_elements ) const
{
    // Sanity check.
    assert( target_elements.size() == element_count_ );
    ...
    size_t const target_pop_count = genesis::utils::bit::pop_count( target_elements );
    ...
    for( size_t i = 1 + element_count_; i < colors_.size(); ++i ) {
        ...
        if( ! genesis::utils::bit::is_superset( colors_[i].elements, target_elements )) {
            continue;
        }

        // See if this is a new minimal hit; if so, update it.
        auto const popcnt = genesis::utils::bit::pop_count( colors_[i].elements );
    ...
    assert( min_index == 0 || min_pop_count >= genesis::utils::bit::pop_count( target_elements ));
