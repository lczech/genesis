...
public:
    ...
    ReferenceGenome()
    {
        guard_ = genesis::utils::core::make_unique<std::mutex>();
    }
    ...
    inline char get_base( const_iterator it, size_t position, bool to_upper = true ) const
    {
        ...
        if( to_upper ) {
            return genesis::utils::text::to_upper( ref_seq[ position - 1 ]);
        }
    ...
    */
    const_reference add( Sequence&& seq, bool also_look_up_first_word = true )
    {
        ...
        auto const label2 = genesis::utils::text::split( seq.label(), "\t ", true )[0];
