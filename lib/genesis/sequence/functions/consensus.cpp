...
std::string consensus_sequence_template(
    ...
    if( counts.characters() != "ACGT" ) {
        throw std::runtime_error(
            "Computation of consensus sequences only possible "
    ...
    for( size_t site_idx = 0; site_idx < counts.length(); ++site_idx ) {
        ...
        // still need to treat gaps separately.
        ...
        result += char_selector( counts_map, counts_sum, gap_count );
    }
...
std::string consensus_sequence_with_ambiguities(
    ...
    auto selector = [&](
        ...
        if( counts_map[0].first > 0 ) {
            ...
            // to it...).
            assert( ! allow_gaps || counts.added_sequences_count() == 0 );
        }
...
std::string consensus_sequence_with_threshold(
    ...
    auto selector = [&](
        ...
        auto const gap_fraction = static_cast<double>( gap_count ) / static_cast<double>( counts_sum );
        if(
            ( gap_count == seq_count ) ||
...
std::string consensus_sequence_cavener(
    ...
    if( counts.characters() != "ACGT" ) {
        throw std::runtime_error(
            "Computation of consensus_sequence_cavener() only possible "
            "for nucleic acid codes (ACGT)."
        );
    }
    ...
    for( size_t site_idx = 0; site_idx < counts.length(); ++site_idx ) {
        ...
        if( counts_map[0].first > 0 ) {
            ...
            if(( counts_map[0].first > counts_sum / 2 ) && ( counts_map[0].first > 2 * counts_map[1].first )) {
                ambiguity_codes = std::string( 1, counts_map[0].second );

            // If the first two freqs > 75% (and both < 50%, which was checked above), use dual code.
            } else if( counts_map[0].first + counts_map[0].first > 3 * counts_sum / 4 ) {
                ambiguity_codes
