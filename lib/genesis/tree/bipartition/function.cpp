...
std::vector<Bipartition> bipartition_set( Tree const& tree )
{
    ...
    for( auto it : postorder(tree) ) {
        ...
        auto bp = Bipartition( it.link(), genesis::util::bit::Bitvector( num_leaves ));
...
std::vector<size_t> node_to_leaf_map( Tree const& tree )
{
    ...
    if( sort_leaves ) {
        ...
        auto const ordered = genesis::util::core::sort_indices( node_names.begin(), node_names.end() );
...
{
    auto const node_to_leafs = node_to_leaf_map( tree );
    genesis::util::bit::Bitvector result( leaf_node_count( tree ));
...
Bipartition find_smallest_subtree(
    ...
    for( Bipartition const& bip : bipartitions ) {
        ...
        if( genesis::util::bit::is_subset( comp, bip.leaf_nodes() )) {
            if( min_count == 0 || pop_count(bip.leaf_nodes()) < min_count ) {
                best_bip = bip;
                min_count = pop_count(best_bip.leaf_nodes());
            }
        }
        if( genesis::util::bit::is_subset( comp, inverted ) ) {
...
std::vector<size_t> find_monophyletic_subtree_edges(
    ...
    auto result_edges = genesis::util::bit::Bitvector( tree.edge_count() );
