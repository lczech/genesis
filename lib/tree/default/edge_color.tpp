...
std::vector<Color> edge_color_branch_length_gradient( TreeType const& tree, bool zero_based )
{
    // Init the result vector with the min head color (green) for each edge.
    auto ret = std::vector<Color>( tree.edge_count(), heat_gradient(0.0) );
    ...
    for (auto it = tree.begin_edges(); it != tree.end_edges(); ++it) {
        ...
        ret[edge.index()] = heat_gradient(val);
    }
    return ret;
}
