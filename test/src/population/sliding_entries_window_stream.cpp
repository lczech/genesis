...
void test_sliding_entries_stream_( WindowStream& win_it, size_t count )
{
...
void test_window_stream_sliding_entries_direct( size_t count )
{
    ...
    auto win_it = make_default_sliding_entries_window_stream(
        pileup_begin, pileup_end, count
    );

    // Run the tests.
    test_sliding_entries_stream_( win_it, count );
...
TEST( WindowStream, SlidingEntriesDirect )
{
    test_window_stream_sliding_entries_direct(  9000 );
    test_window_stream_sliding_entries_direct( 10000 );
}

void test_window_stream_sliding_entries_lambda( size_t count )
{
    // Skip test if no data availabe.
    NEEDS_TEST_DATA;
    std::string const infile = environment->data_dir + "population/78.pileup.gz";
...
TEST( WindowStream, SlidingEntriesLambda )
{
    test_window_stream_sliding_entries_lambda(  9000 );
    test_window_stream_sliding_entries_lambda( 10000 );
}

void run_sliding_entries_window_view_variant_test_( VariantWindowViewStream& win_it )
{
    size_t window_cnt = 0;
    for( auto it = win_it.begin(); it != win_it.end(); ++it ) {
        auto const& window = *it;
...
TEST( WindowStream, SlidingEntriesWindowView )
{
    // Skip test if no data availabe.
    NEEDS_TEST_DATA;
    std::string const infile = environment->data_dir + "population/78.pileup.gz";
    ...
    // Create a window iterator based on the Generic Input Stream.
    auto win_it = make_default_sliding_entries_window_view_stream(
...
TEST( WindowStream, SlidingEntriesEmpty )
{
    // Skip test if no data availabe.
    NEEDS_TEST_DATA;
