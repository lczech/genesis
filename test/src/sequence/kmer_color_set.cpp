...
TEST( KmerColorSet, Basics )
{
    ...
    for( size_t p = 1; p <= 17; ++p ) {
        ...
        for( size_t i = 0; i < initial_cset_size; ++i ) {
            ...
            EXPECT_EQ( i, color.index );
            EXPECT_EQ( i, cset.find_color( color.elements ));
        }
        ...
        if( p > 2 ) {
            ...
            EXPECT_EQ( 0, cset.find_color( non_existing ));
        }

        // Test that all primary colors can be retrieved.
        // That should also not change our color set size
        for( size_t i = 0; i < p; ++i ) {
            EXPECT_EQ( i + 1, cset.lookup_and_update( 0, i ));
        }
        EXPECT_EQ( initial_cset_size, cset.get_color_list().size() );

        auto const type_counts = count_kmer_color_set_color_types( cset );
        EXPECT_EQ(   1, type_counts[0] );
        EXPECT_EQ( p  , type_counts[1] );
        EXPECT_EQ( p-1, type_counts[2] );
        EXPECT_EQ(   0, type_counts[3] );
...
TEST( KmerColorSet, Example )
{
    ...
    // Set up a manual example that tests every code path of the lookup_and_update function
    ...
    for( size_t i = 0; i < cset.get_color_list().size(); ++i ) {
        ...
        for( size_t b = 0; b < p; ++b ) {
            if( color.elements.get(b) ) {
                EXPECT_EQ( i, cset.lookup_and_update( i, b ));
            }
        }
    }
    ...
    for( size_t i = 0; i < p; ++i ) {
        ...
        if( i % 2 == 0 ) {
            EXPECT_EQ( 11 + i / 2, cset.lookup_and_update( i + 1, i + 1 ));
        } else {
            EXPECT_EQ( 11 + i / 2, cset.lookup_and_update( i + 1, i - 1 ));
        }
    }

    // We have only asked for existing colors so far.
    // Check that all the above did not add any extra colors yet.
    EXPECT_EQ( initial_cset_size, cset.get_color_list().size() );
    auto type_counts = count_kmer_color_set_color_types( cset );
    EXPECT_EQ(   1, type_counts[0] );
    EXPECT_EQ( p  , type_counts[1] );
    EXPECT_EQ( p-1, type_counts[2] );
    EXPECT_EQ(   0, type_counts[3] );
    ...
    for( size_t i = 11; i <= 15; ++i ) {
        ...
        for( size_t b = 0; b < p; ++b ) {
            ...
            EXPECT_EQ( exp_idx, cset.lookup_and_update( i, b ));
            EXPECT_EQ( exp_idx, cset.lookup_and_update( i, b ));
        }
    }
    ...
    type_counts = count_kmer_color_set_color_types( cset );
    EXPECT_EQ( 1, type_counts[0] );
    EXPECT_EQ( p, type_counts[1] );
    EXPECT_EQ( p-1 + 5 * 8, type_counts[2] );
    EXPECT_EQ( 0, type_counts[3] );

    // At this point, we have exactly 60 colors: 1 empty, 10 primary,  9 initial secondary,
    // and 40 new secondary. We have set the real color limit to 59, so that we have exatly
    ...
    // indices 16 and 17 above, and adding another bit to either of them yields new colors.
    // These will be imaginary, and the closest fit will either be color 18 or 19.
    ...
    // We added 12 imaginary colors now.
    EXPECT_EQ( initial_cset_size + 5 * 8 + 12, cset.get_color_list().size() );
    type_counts = count_kmer_color_set_color_types( cset );
    EXPECT_EQ( 1, type_counts[0] );
    EXPECT_EQ( p, type_counts[1] );
    EXPECT_EQ( p-1 + 5 * 8, type_counts[2] );
    EXPECT_EQ( 12, type_counts[3] );
...
TEST( KmerColorSet, Random )
{
    ...
    for( size_t i = 0; i < n; ++i ) {
        // Pick a random entry, and a random bit, and look it up.
        // We are currently also picking imaginary colors, and update them.
        // In a typical use case, that would not happen, but as far as I can tell right now,
        // it still should work as expected, so let's test it anyway.
        // Otherwise, we could limit this to only look up colors at real indices
        // (the commented line below), in which case however we would not generate
        // all possible combinations of bits for the colors.
    ...
    // every color in the 8 bit range that we are using here.
    // Most of the imaginary ones will point to the all-set color at index 15,
    // while there will be eight each for indices 13 and 14.
    EXPECT_EQ( 256, cset.get_color_list().size() );
    auto img_idx_counts = std::vector<size_t>( 16, 0 );
    for( size_t c = 16; c < 256; ++c ) {
        EXPECT_LE( 13, cset.get_color_at( c ).index );
        EXPECT_GE( 15, cset.get_color_at( c ).index );
        ++img_idx_counts[cset.get_color_at( c ).index];
    }
    EXPECT_EQ(   8, img_idx_counts[13] );
    EXPECT_EQ(   8, img_idx_counts[14] );
    EXPECT_EQ( 224, img_idx_counts[15] );

    // Test that we have exactly as many colors of each type as expected
    type_counts = count_kmer_color_set_color_types( cset );
    EXPECT_EQ( 1, type_counts[0] );
    EXPECT_EQ( p, type_counts[1] );
    EXPECT_EQ( p-1, type_counts[2] );
    EXPECT_EQ( 240, type_counts[3] );
