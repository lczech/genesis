...
{
    InputStream instr( genesis::util::core::make_unique< StringInputSource >( str ));
...
TEST(InputStream, FileReading)
{
    ...
    InputStream instr( genesis::util::core::make_unique< FileInputSource >( infile ));
...
RandomFuzzyLines make_random_fuzzy_lines_( size_t const n_lines, size_t const max_len = 100 )
{
    ...
    // LOG_DBG << genesis::util::join( result.line_lengths );
    result.text = ss.str();
    return result;
}
...
TEST( InputStream, GetLineLargeFile )
{
    ...
    std::string tmpfile = environment->data_dir + "util/large_file.txt";
...
void test_input_stream_large_fuzzy_()
{
    ...
    // LOG_DBG << genesis::util::join( line_lengths );
...
TEST( InputStream, ParseInt )
{
    ...
    std::string tmpfile = environment->data_dir + "util/ints.txt";
    std::ofstream out{ tmpfile };
    ASSERT_TRUE( out );
