...
TEST( FileSystem, FileExists )
{
    ...
    EXPECT_TRUE( is_file( environment->data_dir + "util/csv/table.csv" ));
    EXPECT_FALSE( is_file( "/road/to/nowhere" ));

    // Test existing directory. Should be false, but no throw.
    EXPECT_FALSE( is_file( environment->data_dir + "util/csv/" ));
}
...
TEST( File, ReadLines )
{
    ...
    std::string const infile = environment->data_dir + "util/csv/table.csv";
