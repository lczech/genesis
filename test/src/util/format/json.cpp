...
TEST( Json, ReadWrite )
{
    ...
    auto deserialized = JsonReader().read( from_string( serialized ));
...
TEST( Json, ParsingFails )
{
    ...
    std::string data_dir = environment->data_dir + "util/json/";
    auto fail_files = dir_list_files( data_dir, true, "fail.*.jtest" );
    ASSERT_EQ( 24, fail_files.size() );

    for( auto const& fail_file : fail_files ) {
        // LOG_DBG << fail_file;
        EXPECT_ANY_THROW( reader.read( from_file( fail_file )));
    }
}
...
TEST( Json, ParsingPasses )
{
    ...
    auto reader = JsonReader();
    std::string data_dir = environment->data_dir + "util/json/";
    auto pass_files = dir_list_files( data_dir, true, "pass.*.jtest" );

    for( auto c : pass_files ) {
        std::cout << c << "\n";
    }
...
//     genesis::util::InputStream is( genesis::util::core::make_unique< genesis::util::FileInputSource >( inputfile ));
//
//     auto reader = JsonReader();
