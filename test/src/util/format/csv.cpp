...
TEST( Csv, ReaderDefault )
{
    ...
    infile = environment->data_dir + "util/csv/simple.csv";
    ...
    infile = environment->data_dir + "util/csv/complex.csv";
    table  = reader.read( from_file( infile ));
...
TEST( Csv, ReaderTabulatedEscaped )
{
    ...
    infile = environment->data_dir + "util/csv/tab_esc.csv";
    table  = reader.read( from_file( infile ));
...
TEST( Csv, ReaderCommentEmpty )
{
    ...
    infile = environment->data_dir + "util/csv/comment_empty.csv";
    table  = reader.read( from_file( infile ));
...
TEST( Csv, InputIterator )
{
    ...
    {
        size_t count = 0;
        auto line_it = CsvInputIterator( from_file( environment->data_dir + "util/csv/complex.csv" ));
        while( line_it ) {
