...
void genesis_util_io_input_source_add_ons(py::module &m)
{
    // ---------------------------------------------------------------------
    //   from_file
    // ---------------------------------------------------------------------
    ...
    m.def("from_file",
        [Path, TextIOBase](py::object src) -> std::shared_ptr<genesis::util::io::BaseInputSource>
        {
            // 1) string path
            if( py::isinstance<py::str>( src )) {
                return genesis::util::io::from_file(src.cast<std::string>());
            }
            // 2) pathlib.Path
            if( py::isinstance( src, Path )) {
                std::string p = src.attr("as_posix")().cast<std::string>();
                return genesis::util::io::from_file(p);
            }
            ...
            if( py::hasattr( src, "read" )) {
                ...
                auto original_src = genesis::util::io::from_stream(holder_sp->stream);
                ...
                return std::shared_ptr<genesis::util::io::BaseInputSource>(
                    joint,
                    joint->first.get()
                );
            }
            ...
            //     return genesis::util::io::from_string(std::move(data));
            // }
            throw std::invalid_argument(
        ...
        "C++: genesis::util::io::from_file(const std::string &) --> class std::shared_ptr<class genesis::util::io::BaseInputSource>"
    );
    ...
    //     source = util.io.from_string(data)
    //
    ...
    m.def("from_string",
        [](py::object src) -> std::shared_ptr<genesis::util::io::BaseInputSource>
        {
            // 1) Simple string
            if( py::isinstance<py::str>( src )) {
                ...
                return genesis::util::io::from_string(src.cast<std::string>());
            }
            ...
            if( py::isinstance<py::buffer>( src )) {
                ...
                auto original_src = genesis::util::io::from_string_view( holder_sp->sv );
                ...
                // which shares ownership of both the target and the holder,
                // meaning that Python now owns both objects.
                std::shared_ptr<genesis::util::io::BaseInputSource> ret(
                    joint,
                    joint->first.get()
                );
                return ret;
        ...
        "sources.\n\nC++: genesis::util::io::from_string(const std::string &) --> class "
