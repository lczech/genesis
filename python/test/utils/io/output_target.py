...
class TestGenesisUtilsIoOutputTarget(unittest.TestCase):
    def setUp(self):
...
class TestGenesisUtilsIoOutputContext(unittest.TestCase):
    def setUp(self):
        # Prepare some known content and a temp file path
        self.content = "Hello, world!\nThis is a test.\n"
        self.path = ""
    ...
    def test_to_string_target(self):
        # 5) Use the convenience to_string() target
        with utils.io.to_string() as target:
            utils.io.write_output_target(self.content, target)
    ...
    def test_to_string_stringio(self):
        # 6) Use a StringIO object as target
        buf = io.StringIO()
        with utils.io.to_string(buf) as target:
            utils.io.write_output_target(self.content, target)
        self.assertEqual(buf.getvalue(), self.content)
...
class TestGenesisUtilsIoOutputGzip(unittest.TestCase):
    ...
    def test_file_compress_stream(self):
        ...
        gzip_level = utils.io.GzipCompressionLevel.kDefaultCompression
        with utils.io.to_file(tmp_file, gzip_level) as target:
            utils.io.write_output_target(raw_data, target)
    ...
    def test_file_compress_stream_func(self):
        ...
        # Read it back in again and see if it worked
        zip_data = utils.io.read_input_source( utils.io.from_file( tmp_name ))
        self.assertEqual(raw_data, zip_data)
